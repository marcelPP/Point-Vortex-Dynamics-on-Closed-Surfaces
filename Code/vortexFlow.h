...
void vortexVelocity(
    ...
    for( int forceId = 0; forceId < nrOfVortices; forceId++){
        ...
        if (isVortex && dist<pow(10,-5) ){ continue; }
        ...
        if( geometryType == "plane"){
            ...
            vector sphereVel = cross(P,vortexP)/(1-dot(P,vortexP));                  
    ...
    if(geometryType == "general"){
		...
		// get vorticity of current vortex
		float currVorticity = attrib(geo,"point","vorticity",vortexId);
        ...
        vector confVel = -currVorticity/h*cross(P,gradh); 
        //confVel = set(0,0,0); //constant test 
        //confVel = gradh/length(gradh)/10; // grad acent test
        //velocity = confVel;
        velocity = (velocity + confVel)/(4*pi*h*h);
